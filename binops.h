...
template <typename T>
void write_le(std::ostream&, const T&);
template <typename T>
T read_be(std::istream&);
template <typename T>
void write_be(std::ostream&, const T&);
...
template <typename T>
static std::vector<T> read_vec_be(std::istream& s) {
...
template <typename T>
static std::vector<T> read_vec_le(std::istream& s) {
...
template <typename T>
static std::vector<T*> read_vec_ptrs_be(std::istream& s) {
	auto count = read_be<std::uint32_t>(s);
	std::vector<T*> result(count);
...
template <typename T>
static std::vector<T*> read_vec_ptrs_le(std::istream& s) {
	auto count = read_le<std::uint32_t>(s);
	std::vector<T*> result(count);
...
template <typename T>
static void write_vec_be(std::ostream& s, const std::vector<T>& vec) {
	write_be<std::uint32_t>(s, vec.size());
	for (const auto& elem : vec)
		write_be<T>(s, elem);
}
template <typename T>
static void write_vec_le(std::ostream& s, const std::vector<T>& vec) {
	write_le<std::uint32_t>(s, vec.size());
	for (const auto& elem : vec)
		write_le<T>(s, elem);
}
template <typename T>
static void write_vec_ptrs_be(std::ostream& s, const std::vector<T*>& vec) {
	write_be<std::uint32_t>(s, vec.size());
	for (const auto& elem : vec)
		write_be<T>(s, *elem);
}
template <typename T>
static void write_vec_ptrs_le(std::ostream& s, const std::vector<T*>& vec) {
	write_le<std::uint32_t>(s, vec.size());
	for (const auto& elem : vec)
		write_le<T>(s, *elem);
}
