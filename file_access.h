...
std::string read_compressed_file(const char* path) {
	...
	if (compressed.size() - 8 != compressed_size) {
		printf("Error opening file %s:\n    Bad compressed size: file was %zi bytes, expected %i.\n", path,
			compressed.size() - 8, compressed_size);
		exit(-1);
	}

	std::string output_buffer(decompressed_size, '\0');
	auto actual_size =
		fastlz_decompress(compressed.data() + 8, compressed_size, output_buffer.data(), output_buffer.size());
	...
	if (actual_size != output_buffer.size()) {
		printf("Error opening file %s:\n    Unexpected decompressed size: file was %zi bytes, expected %i.\n", path,
			output_buffer.size(), actual_size);
		exit(-1);
	}
...
const char* sessionKey = "KnowledgeIsTheHi";
const char* sessionIv = "WhoWouldntGiveEv";

std::string get_session_numbers(const char* path) {
	AES_ctx keydat;
	AES_init_ctx_iv(&keydat, (const uint8_t*)sessionKey, (const uint8_t*)sessionIv);
	std::string bin = read_file(path);
	AES_CTR_xcrypt_buffer(&keydat, (uint8_t*)bin.data(), bin.size());
	return bin;
}